...
export const addUserToMap = async (userId, line) => {
  const oldMap = await getUserMap()
  saveUserMap({...oldMap, [userId]: line})
}
export function getLineByLinea(linea) {
  if (isNaN(linea)) {
    console.log('The ID you entered cannot be found. Please try another ID.');
    return;
  }
...
const getValueByKey = key => {
  const value = usersMap[key];
  if (!value) {
   // console.log("No se encuentra el ID que pusiste. Inténtelo con otro ID.");
  } else {
    return value;
  }
};
...
//console.log(getUserById('fwgxeogpk'))
